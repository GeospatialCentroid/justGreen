...
# --- NDVI & Rankings ---
cityNDVI <- round(cH$meanNDVI_500m, 3)
ndvi_sorted <- cityHealth |>
  arrange(desc(meanNDVI_500m)) |>
...
get_ci_string <- function(val_1, val_2) {
  # Calculate absolute values and sort to ensure low-to-high order
  vals <- sort(abs(round(c(val_1, val_2), 0)))
  l <- format(vals[1], big.mark=",")
  u <- format(vals[2], big.mark=",")
  paste0(l, " – ", u)
}
...
kable(health_df, col.names = c("Outcome", "Annual Cases (Estimate)", "Rate per 100,000", "95% Confidence Interval")) %>%
...
# Prepare Data
ct_sf_clean <- ct_sf %>% filter(!is.na(meanNDVI_500m), !is.na(popOver20_2023))

# Calculate Priority (Low NDVI * High Pop)
ct_sf_clean <- ct_sf_clean %>%
  mutate(priority_score = (1 - meanNDVI_500m) * popOver20_2023) %>%
...
priority_table <- top_5 %>%
  sf::st_drop_geometry() %>%
  select(GEOID, meanNDVI_500m, popOver20_2023) %>% 
  mutate(Rank = 1:5, Area = "Neighborhood/Street") %>%
  select(Rank, GEOID, Area, meanNDVI_500m, popOver20_2023)
...
equity_summary <- ct_equity %>%
  ...
  summarise(
    ...
    Mean_NDVI = mean(meanNDVI_500m, na.rm=T),
...
<div class="footer-banner">
  <img src="assets/rojosLogo.png" height="80px">
  <span>Rojos Lab - Geospatial Centroid<br>Colorado State University © January 2026</span>
  <img src="assets/centroid_white_gray_logo_CROPPED.png" height="60px">
</div>
